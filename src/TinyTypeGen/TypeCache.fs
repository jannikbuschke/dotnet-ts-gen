...
module TypeCache =
  ...
  type Dict<'a, 'b when 'a: not null> = System.Collections.Concurrent.ConcurrentDictionary<'a, 'b>
      ...
      cache.GetOrAdd(
        ...
        fun ty ->
          if ty.IsGenericType && ty.GetGenericTypeDefinition() = listTy then
            ...
            TypeKind.Map
          elif ty.IsGenericType && ty.GetGenericTypeDefinition() = optionTy then
            TypeKind.Option
  ...
  let isAnonRecord ty =
    getKind ty = TypeKind.Record && ty.Name.StartsWith "<>f__AnonymousType"
