...
type EdgeId =
  ...
  static member FromNodeIds(src: NodeId, target: NodeId) =
    EdgeId(src.Val().ToString() + "_" + target.Val().ToString())
...
type Graph =
  ...
  member this.GetNodeStatus(nodeId: NodeId) =
    let node = this.Nodes |> List.find (fun x -> x.NodeId = nodeId)

    node.Status

  member this.GetNodesWithStatus(status: WorkflowNodeStatus) =
    this.Nodes |> List.filter (fun x -> x.Status = status)
  ...
  member this.GetSourceNode(edge: Edge) =
    this.Nodes |> List.find (fun n -> n.NodeId = edge.Source)

  member this.GetTargetNode(edge: Edge) =
    this.Nodes |> List.find (fun n -> n.NodeId = edge.Target)

  member this.IncomingEdges(nodeId: NodeId) =
    this.Edges |> List.filter (fun e -> e.Target = nodeId)

  member this.OutgoingDefaultEdges(nodeId: NodeId) =
    this.Edges |> List.filter (fun e -> e.Source = nodeId)

  member this.Predecessors(nodeId: NodeId) =
    nodeId |> this.IncomingEdges |> List.map this.GetSourceNode

  member this.Successors(nodeId: NodeId) =
    nodeId |> this.OutgoingDefaultEdges |> List.map this.GetTargetNode
...
let getAllActiveNodes (nodes: #IWorkflowNode list) =
  nodes |> List.filter (fun x -> x.Status = WorkflowNodeStatus.Active)

let getAllPendingNodes (nodes: #IWorkflowNode list) =
  nodes |> List.filter (fun x -> x.Status = WorkflowNodeStatus.Pending)

let getAllCompletedNodes (nodes: #IWorkflowNode list) =
  nodes |> List.filter (fun x -> x.Status = WorkflowNodeStatus.Completed)
...
let decideWorkflowCommand (state: WorkflowState) (command: WorkflowCommand) =
  ...
  | WorkflowCommand.Trigger nodeId ->
    let trigger = state.Graph.Triggers |> List.tryFind (fun n -> n = nodeId)
  ...
  | WorkflowCommand.ActivateNodes ->
    ...
    let successors =
      completedNodes |> List.collect (fun x -> state.Graph.Successors(x.NodeId))

    let pendingNodes =
      successors |> List.filter (fun x -> x.Status = WorkflowNodeStatus.Pending)

    pendingNodes
    |> List.choose (fun node -> WorkflowEngineEvent.NodeEvent(NodeEvent.Activated, node.NodeId) |> Some)
    |> Result.Ok
